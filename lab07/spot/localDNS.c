...
int sendb,recvb;
char host[MAX];
recvb=recvfrom(sockfd,(char*)host,sizeof(host),0,(struct
...
sendb=sendto(rootsock,(const char*)host,strlen(host),0,(const struct
sockaddr*)&rootaddr,rsize);
...
sendto(sockfd,(const char*)IP,strlen(IP),0,(const struct
sockaddr*)&cliaddr,size);
