...
recvb=recvfrom(sockfd,(char*)query,sizeof(query),0,(struct
...
sendb=sendto(Authsock,(const char*)query,strlen(query),0,(const struct
sockaddr*)&Authaddr,rsize);
printf("4.Query sent to Auth server\n");
char IP[MAX];
recvb=recvfrom(Authsock,(char*)IP,sizeof(IP),0,NULL,NULL);
IP[recvb]='\0';
sendb=sendto(sockfd,(const char*)IP,strlen(IP),0,(const struct
sockaddr*)&cliaddr,size);
...
printf("TLD DNS server running at PORT %d\n",TLD);
