...
int main()
{
	...
	printf("num_packet = %d\n", num_packet);
	int drop = -1;
	printf("enter the packet number of the ack to be dropped(0 to %d)\n", num_packet);
	scanf("%d", &drop);
	...
	while (1)
	{
		...
		if (flag != 1)
		{
			if (packet[count].header.ack_flag == -1)
			{
				lost_count = count;
				lost_seq_no = packet[count].header.seq_num;
				printf("the data:%s\n", packet[count].data);
				printf("ack dropped for packet number %d (seq num %d)\n", count, packet[count].header.seq_num);
				sprintf(response, "DropACK %d", lost_seq_no);
